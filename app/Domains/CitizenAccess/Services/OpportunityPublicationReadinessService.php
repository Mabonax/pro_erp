...
class OpportunityPublicationReadinessService
{
    public function evaluate(Opportunity $opportunity): OpportunityPublicationReadiness
    {
        ...
        $checks = [
            $this->check('is_active', 'Opportunity is active', (bool) $opportunity->is_active, 'Opportunity must be active.'),
            $this->check('public_slug', 'Public slug/code set', filled($opportunity->public_slug), 'Set a public slug/code.'),
            $this->check('public_title', 'Public title set', filled($opportunity->public_title), 'Set a public title.'),
            $this->check('is_published', 'Published flag enabled', (bool) $opportunity->is_published, 'The offering has not been published.'),
            $this->check('status', 'Lifecycle status is published', $opportunity->status === 'published', 'Lifecycle status must be published.'),
            $this->check('archived_at', 'Offering is not archived', blank($opportunity->archived_at), 'Archived offerings cannot be public.'),
            $this->check('service_stream_id', 'Active service stream assigned', $this->hasActiveServiceStream($opportunity), 'Choose an active service stream.'),
            $this->check('program_id', 'Programme assigned', filled($opportunity->program_id), 'Choose a programme.'),
            $this->check('project_id', 'Project assigned to programme', $this->hasMatchingProject($opportunity), 'Choose a project that belongs to the selected programme.'),
            $this->check('project_location_id', 'Project location assigned to project', $this->hasMatchingProjectLocation($opportunity), 'Choose a project location for the selected project.'),
            $this->check('requirement_template_id', 'Published requirement template assigned', $this->hasPublishedRequirementTemplate($opportunity), 'Choose a requirement template with a published version.'),
            $this->check('service_pathway_version_id', 'Active pathway version valid where selected', $this->hasValidPathwayVersion($opportunity), 'Choose an active pathway version linked to the pathway.'),
            $this->check('dates', 'Required dates valid', $this->hasValidDates($opportunity), 'Closing date must be on or after opening date.'),
        ];
    ...
    private function check(string $field, string $label, bool $passes, string $message): array
    {
        return compact('field', 'label', 'passes', 'message');
    }
