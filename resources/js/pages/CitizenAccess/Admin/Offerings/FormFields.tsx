        ...
        <form onSubmit={submit} className="grid gap-6">
            <section className="grid gap-4">
                ...
                <div className="grid gap-3 md:grid-cols-2 xl:grid-cols-4">
                    ...
                    <Select label="Status" value={data.status} error={errors.status} onChange={(value) => setData('status', value)} options={[['draft', 'Draft'], ['ready', 'Ready'], ['published', 'Published (use Publish Offering action)'], ['unpublished', 'Unpublished'], ['archived', 'Archived']]} />
            ...
            <section className="grid gap-4">
                ...
                <div className="grid gap-3 md:grid-cols-2 xl:grid-cols-4">
                    ...
                    <Select label="Program" value={data.program_id} error={errors.program_id} onChange={(value) => { setData('program_id', value); setData('project_id', ''); setData('project_location_id', ''); }} options={options.programs.map((item) => [item.id, item.title ?? item.name ?? String(item.id)])} />
